...
The MVP makes **source selection, a high-fidelity studio UI, camera/microphone permissions, movable overlay placement, local reaction-track recording, and a clean screen-recording scene** work on device. For a personal-use merged video immediately, the user starts the device’s own screen recorder with microphone enabled, then starts Reel Reactor’s clean scene; the resulting screen capture contains the source clip and camera bubble together. A native, file-level video composition/export pipeline remains a later enhancement. Direct playback/downloading of Instagram, Facebook, and TikTok content is not assumed: the owner should only use videos they have rights to reuse and import a local copy where the social platform does not provide a shareable file.
